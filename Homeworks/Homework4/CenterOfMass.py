...
MW_COM = CenterOfMass("MW_000.txt", 1)
M31_COM = CenterOfMass("M31_000.txt", 1)
M33_COM = CenterOfMass("M33_000.txt", 1)
...
PSEP = MW_COM_p - M31_COM_p
PSEPMAG = np.sqrt(PSEP[0]**2 + PSEP[1]**2 + PSEP[2]**2)
PSEPMAG = np.round(PSEPMAG, 3)
print(PSEPMAG)
...
VSEP = M31_COM_v - MW_COM_v
VSEPMAG = np.sqrt(VSEP[0]**2 + VSEP[1]**2 + VSEP[2]**2)
VSEPMAG = np.round(VSEPMAG, 3)
print(VSEPMAG)
...
VSEP = M33_COM_v - M31_COM_v
VSEPMAG = np.sqrt(VSEP[0]**2 + VSEP[1]**2 + VSEP[2]**2)
VSEPMAG = np.round(VSEPMAG, 3)
print(VSEPMAG)


# Determining the COM is important throught the iterative process because we need to know when the two galaxies will fully overlap each other and become one galaxy. Also, to see what other galactical colissions that may happen in the process.
